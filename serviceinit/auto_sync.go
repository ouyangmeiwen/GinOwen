...
func(s AutoService) CusSyncDatabase() error {
	...
	var offset int // 用于分页
		...
		// Syncing Bookcaseinfo model data
		offset = 0
		for {
			var BookcaseinfoData []model.Bookcaseinfo
			// 分页查询数据，不依赖排序字段
			if err := global.OWEN_DBList["from"].Offset(offset).Limit(batchSize).Find(&BookcaseinfoData).Error; err != nil {
				log.Fatalf("Failed to query table: Bookcaseinfo")
				return err
			}

			// 如果没有更多数据，退出循环
			if len(BookcaseinfoData) == 0 {
				break
			}
			...
			// 将数据插入到 db2
			if err := global.OWEN_DBList["to"].CreateInBatches(BookcaseinfoData, batchSize).Error; err != nil {
				log.Fatalf("Failed to insert table: Bookcaseinfo")
				return err
			}
		}
		...
		// Syncing Itemloc010301 model data
		offset = 0
		for {
			var Itemloc010301Data []model.Itemloc010301
			// 分页查询数据，不依赖排序字段
			if err := global.OWEN_DBList["from"].Offset(offset).Limit(batchSize).Find(&Itemloc010301Data).Error; err != nil {
				log.Fatalf("Failed to query table: Itemloc010301")
				return err
			}

			// 如果没有更多数据，退出循环
			if len(Itemloc010301Data) == 0 {
				break
			}
			...
			// 将数据插入到 db2
			if err := global.OWEN_DBList["to"].CreateInBatches(Itemloc010301Data, batchSize).Error; err != nil {
				log.Fatalf("Failed to insert table: Itemloc010301")
				return err
			}
		}
		
		// Syncing Item model data
		offset = 0
		for {
			var ItemData []model.Item
			// 分页查询数据，不依赖排序字段
			if err := global.OWEN_DBList["from"].Offset(offset).Limit(batchSize).Find(&ItemData).Error; err != nil {
				log.Fatalf("Failed to query table: Item")
				return err
			}
			...
			// 将数据插入到 db2
			if err := global.OWEN_DBList["to"].CreateInBatches(ItemData, batchSize).Error; err != nil {
				log.Fatalf("Failed to insert table: Item")
				return err
			}
		}
		...
		// Syncing Libpointrobot model data
		offset = 0
		for {
			var LibpointrobotData []model.Libpointrobot
			// 分页查询数据，不依赖排序字段
			if err := global.OWEN_DBList["from"].Offset(offset).Limit(batchSize).Find(&LibpointrobotData).Error; err != nil {
				log.Fatalf("Failed to query table: Libpointrobot")
				return err
			}

			// 如果没有更多数据，退出循环
			if len(LibpointrobotData) == 0 {
				break
			}
			...
			// 将数据插入到 db2
			if err := global.OWEN_DBList["to"].CreateInBatches(LibpointrobotData, batchSize).Error; err != nil {
				log.Fatalf("Failed to insert table: Libpointrobot")
				return err
			}
		}
		...
		// Syncing Libshelfpoint model data
		offset = 0
		for {
			var LibshelfpointData []model.Libshelfpoint
			// 分页查询数据，不依赖排序字段
			if err := global.OWEN_DBList["from"].Offset(offset).Limit(batchSize).Find(&LibshelfpointData).Error; err != nil {
				log.Fatalf("Failed to query table: Libshelfpoint")
				return err
			}

			// 如果没有更多数据，退出循环
			if len(LibshelfpointData) == 0 {
				break
			}
			...
			// 将数据插入到 db2
			if err := global.OWEN_DBList["to"].CreateInBatches(LibshelfpointData, batchSize).Error; err != nil {
				log.Fatalf("Failed to insert table: Libshelfpoint")
				return err
			}
		}
		...
		// Syncing OwenAuditlog model data
		offset = 0
		for {
			var OwenAuditlogData []model.OwenAuditlog
			// 分页查询数据，不依赖排序字段
			if err := global.OWEN_DBList["from"].Offset(offset).Limit(batchSize).Find(&OwenAuditlogData).Error; err != nil {
				log.Fatalf("Failed to query table: OwenAuditlog")
				return err
			}

			// 如果没有更多数据，退出循环
			if len(OwenAuditlogData) == 0 {
				break
			}
			...
			// 将数据插入到 db2
			if err := global.OWEN_DBList["to"].CreateInBatches(OwenAuditlogData, batchSize).Error; err != nil {
				log.Fatalf("Failed to insert table: OwenAuditlog")
				return err
			}
		}
		
		// Syncing OwenRole model data
		offset = 0
		for {
			var OwenRoleData []model.OwenRole
			// 分页查询数据，不依赖排序字段
			if err := global.OWEN_DBList["from"].Offset(offset).Limit(batchSize).Find(&OwenRoleData).Error; err != nil {
				log.Fatalf("Failed to query table: OwenRole")
				return err
			}

			// 如果没有更多数据，退出循环
			if len(OwenRoleData) == 0 {
				break
			}
			...
			// 将数据插入到 db2
			if err := global.OWEN_DBList["to"].CreateInBatches(OwenRoleData, batchSize).Error; err != nil {
				log.Fatalf("Failed to insert table: OwenRole")
				return err
			}
		}
		
		// Syncing OwenScheduledtask model data
		offset = 0
		for {
			var OwenScheduledtaskData []model.OwenScheduledtask
			// 分页查询数据，不依赖排序字段
			if err := global.OWEN_DBList["from"].Offset(offset).Limit(batchSize).Find(&OwenScheduledtaskData).Error; err != nil {
				log.Fatalf("Failed to query table: OwenScheduledtask")
				return err
			}

			// 如果没有更多数据，退出循环
			if len(OwenScheduledtaskData) == 0 {
				break
			}
			...
			// 将数据插入到 db2
			if err := global.OWEN_DBList["to"].CreateInBatches(OwenScheduledtaskData, batchSize).Error; err != nil {
				log.Fatalf("Failed to insert table: OwenScheduledtask")
				return err
			}
		}
		
		// Syncing OwenUser model data
		offset = 0
		for {
			var OwenUserData []model.OwenUser
			// 分页查询数据，不依赖排序字段
			if err := global.OWEN_DBList["from"].Offset(offset).Limit(batchSize).Find(&OwenUserData).Error; err != nil {
				log.Fatalf("Failed to query table: OwenUser")
				return err
			}

			// 如果没有更多数据，退出循环
			if len(OwenUserData) == 0 {
				break
			}
			...
			// 将数据插入到 db2
			if err := global.OWEN_DBList["to"].CreateInBatches(OwenUserData, batchSize).Error; err != nil {
				log.Fatalf("Failed to insert table: OwenUser")
				return err
			}
		}
