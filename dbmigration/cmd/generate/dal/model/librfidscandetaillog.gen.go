// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.
// Code generated by gorm.io/gen. DO NOT EDIT.

package model

import (
	"time"
)

const TableNameLibrfidscandetaillog = "librfidscandetaillog"

// Librfidscandetaillog mapped from table <librfidscandetaillog>
type Librfidscandetaillog struct {
	ID            string     `gorm:"column:Id;type:varchar(32);primaryKey" json:"Id"`
	CreationTime  *time.Time `gorm:"column:CreationTime;type:datetime(6)" json:"CreationTime"`
	CreatorUserID *int64     `gorm:"column:CreatorUserId;type:bigint" json:"CreatorUserId"`
	ItemTid       *string    `gorm:"column:ItemTid;type:varchar(32)" json:"ItemTid"`
	ItemEpc       *string    `gorm:"column:ItemEpc;type:varchar(32)" json:"ItemEpc"`
	ItemBarcode   *string    `gorm:"column:ItemBarcode;type:varchar(32)" json:"ItemBarcode"`
	Title         *string    `gorm:"column:Title;type:varchar(256)" json:"Title"`
	RfidReaderID  string     `gorm:"column:RfidReaderId;type:varchar(32);not null" json:"RfidReaderId"`
	Antenna       string     `gorm:"column:Antenna;type:varchar(256);not null" json:"Antenna"`
	Remark        *string    `gorm:"column:Remark;type:varchar(256)" json:"Remark"`
	TenantID      int64      `gorm:"column:TenantId;type:int;not null" json:"TenantId"`
	RecordTime    *time.Time `gorm:"column:RecordTime;type:datetime(6)" json:"RecordTime"`
}

// TableName Librfidscandetaillog's table name
func (*Librfidscandetaillog) TableName() string {
	return TableNameLibrfidscandetaillog
}
